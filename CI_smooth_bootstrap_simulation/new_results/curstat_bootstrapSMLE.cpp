...
double  bdf(double A, double B,  int m, double *data0, double *p, double h, double u);
...
{
    double          A,B,c,*data,*data0,*data1,*data2,*grid,*pp,*pp2,step,h,h0;
    double          *tt,*lowbound,*upbound;
    double          *cumw,*cs,*F,*F2,*y,*y2,*SMLE,*SMLE1,*SMLE2,*Fsmooth,*cc,*cc1,*cc2;
    double          **f3,*f4;
    ...
    for (iter=1;iter<=NumIt;iter++)
    {
        ...
        for (i=1;i<=ngrid;i++)
            SMLE[i]=bdf(A,B,m,data0,pp,grid[i],cc[i]*h);
        
        for (i=1;i<=ngrid;i++)
            SMLE1[i]=bdf_conv(A,B,m,data0,pp,grid[i],cc[i]*h,cc[i]*h0);
        
        for (i=1;i<=n;i++)
            Fsmooth[i]=bdf(A,B,m,data0,pp,data[i],cc1[i]*h0);
        ...
        for (iter2=1;iter2<=NumIt2;iter2++)
        {
            ...
            for (i=1;i<npoints;i++)
                ...
                //f3[iter2][i]=(SMLE2[10*i]-SMLE1[10*i])/sqrt(varF(n,n,freq2,y2,0.0,B,data,cc[10*i]*h0,grid[10*i]));
        }
        ...
        for (i=1;i<npoints;i++)
        {
            ...
            lowbound[i] = SMLE[10*i]-bias(grid[10*i],cc[10*i]*h)-f4[above];
            //lowbound[i] = SMLE[10*i]-f4[above]*sqrt(varF(n,n,freq,y,0.0,B,data,cc[10*i]*pow(n,-1.0/5),grid[10*i]));
            //lowbound[i] = SMLE[10*i]-bias(grid[10*i],cc[10*i]*h)-f4[above]*sqrt(varF(n,n,freq,y,0.0,B,data,cc[10*i]*pow(n,-1.0/5),grid[10*i]));
            upbound[i] = SMLE[10*i]-bias(grid[10*i],cc[10*i]*h)-f4[below];
            //upbound[i]= SMLE[10*i]-f4[below]*sqrt(varF(n,n,freq,y,0.0,B,data,cc[10*i]*pow(n,-1.0/5),grid[10*i]));
            //upbound[i]= SMLE[10*i]-bias(grid[10*i],cc[10*i]*h)-f4[below]*sqrt(varF(n,n,freq,y,0.0,B,data,cc[10*i]*pow(n,-1.0/5),grid[10*i]));
    ...
    if (file1_.is_open())
    {
        for (i=1;i<=ngrid;i++)
        {
            ...
            file1_ << setprecision(11) <<  setw(20) << bias(grid[i],cc[i]*h);
            file1_ << "\n";
        }
        file1_.close();
    }
