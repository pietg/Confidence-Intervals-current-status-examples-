...
List ComputeIntervals(int N=1000, int NumB=1000)
{
    ...
    for (i=1;i<npoints;i++)
        //diff[i] = SMLE1[10*i]-SMLE0[10*i];
        //diff[i] = bias(grid[10*i],cc[10*i]*h);
        diff[i]=0;
                
    lowbound[0]=upbound[0]=0;
    ...
    for (iter=1;iter<=NumIt;iter++)
    {
        ...
        for (i=1;i<=ngrid;i++)
            SMLE0[i]=bdf(A,B,m,tt,pp,grid[i],h0);
        
        for (i=1;i<=ngrid;i++)
            SMLE[i]=bdf(A,B,m,tt,pp,grid[i],h);
        
        for (i=1;i<=ngrid;i++)
            SMLE1[i]=bdf_conv(A,B,m,tt,pp,grid[i],h,h0);
        
        for (i=1;i<=n;i++)
            Fsmooth[i]=bdf(A,B,m,tt,pp,data[i],h0);
        
        for (i=1;i<npoints;i++)
            diff[i] = SMLE1[10*i]-SMLE0[10*i];
            //diff[i] = bias(grid[10*i],cc[10*i]*h);
            //diff[i] = bias(grid[10*i],m,data,pp,h,h0);
        ...
        for (iter2=1;iter2<=NumIt2;iter2++)
        {
            ...
            for (i=1;i<=ngrid;i++)
                SMLE2[i]=bdf(A,B,m_bootstrap,data2,pp2,grid[i],h);
            
            
            for (i=1;i<npoints;i++)
                //f3[iter2][i]=SMLE2[10*i]-SMLE1[10*i];
                f3[iter2][i]=(SMLE2[10*i]-SMLE1[10*i])/sqrt(varF(n,n,freq2,y2,0.0,B,data,h,grid[10*i]));
            
            //for (i=1;i<npoints;i++)
                //diff[i] += SMLE2[10*i]-SMLE0[10*i];
                //diff[i] = bias(grid[10*i],h);
                //diff[i] = bias(grid[10*i],m,data,pp,2*h,h0);
        }
        ...
        for (i=1;i<npoints;i++)
        {
            ...
            //lowbound[i] = SMLE[10*i]-f4[above]-bias(grid[10*i],m,tt,pp,2*h,h0);
            lowbound[i] = SMLE[10*i]-f4[above]*sqrt(varF(n,n,freq,y,0.0,B,data,h,grid[10*i]));
            ...
            //upbound[i] = SMLE[10*i]-f4[below]-bias(grid[10*i],m,tt,pp,h,h0);

            upbound[i]= SMLE[10*i]-f4[below]*sqrt(varF(n,n,freq,y,0.0,B,data,h,grid[10*i]));
            
            lowbound[i]=fmax(lowbound[i],lowbound[i-1]);
            upbound[i]=fmax(upbound[i],upbound[i-1]);
            
            lowbound[i]=fmin(lowbound[i],1);
            upbound[i]=fmin(upbound[i],1);
