...
List ComputeIntervals(int N=1000, int NumB=1000)
{
    ...
    for (iter=1;iter<=NumIt;iter++)
    {
        ...
        for (i=1;i<npoints;i++)
        {
            ...
            //lowbound[i] = SMLE[10*i]-f4[above]*sqrt(varF(n,n,freq,y,0.0,B,data,cc[10*i]*h,grid[10*i]))-diff[i];
            ...
            //upbound[i]= SMLE[10*i]-f4[below]*sqrt(varF(n,n,freq,y,0.0,B,data,cc[10*i]*h,grid[10*i]))-diff[i];
